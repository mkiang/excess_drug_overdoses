...
## Retrieve joinpoint results ----
jp_results <-
    left_join(import_jp(
        here::here(
            "joinpoint",
            "educ_alone",
            "educ_imputed_zero_monthly_deaths.data.txt"
        ),
        ctypes = "cccinnncii"
    ),
    monthly_df)  %>%
    dplyr::left_join(
        import_jp(
            here::here(
                "joinpoint",
                "educ_alone",
                "educ_imputed_zero_monthly_deaths.mpc.txt"
            )
        ) %>%
            dplyr::select(
                race,
...
p1 <- ggplot() +
    ...
    scale_color_viridis_d(name = "Education") +
    scale_alpha_manual(
        "Change in slope",
        values = c(.35, 1),
        labels = c(
    ...
    geom_vline(xintercept = as.Date(config::get("forecast_start")),
               linetype = "dotted") +
    facet_wrap( ~ death_cat, scales = "free", ncol = 4)

## Save ----
ggsave(
    here("plots", "figS12_joinpoint_test_plot_by_educ_only.pdf"),
    p1,
    width = 8,
    height = 10,
    scale = 1,
...
ggsave(
    here("plots", "figS12_joinpoint_test_plot_by_educ_only.jpg"),
    p1,
    width = 8,
    height = 10,
    scale = 1,
...
write_csv(
    ...
    here("output", "figS12_data_jp_by_educ.csv")
)
