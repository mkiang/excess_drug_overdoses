...
cume_excess_deaths_hisp <- readRDS(here("data", "cume_excess_deaths.RDS")) %>%
    filter(
        ...
        # race == "all_nonhisp",
        race %in% c("all_nonhisp", "hispanic"), 
        educ != "all", 
        educ != "over25", 
        educ != "under25", 
        date == max(date)
...
abs_cume <- ggplot(
    ...
    geom_errorbarh(height = .2, 
                   ...
                   alpha = .9) +
    geom_point(aes(size = abs(fitted / pop * 100000)),
               position = position_dodge(width = .7),
               alpha = .9) + 
...
per_capita_cume <- ggplot(
    ...
    facet_wrap(~ race_cat)
ggsave(
    here("plots", "figS07_cume_excess_deaths_per_100k_by_educ_and_hisp.pdf"),
    per_capita_cume,
    width = 9.5,
    height = 4,
    scale = 1,
...
ggsave(
    here("plots", "figS07_cume_excess_deaths_per_100k_by_educ_and_hisp.jpg"),
    per_capita_cume,
    width = 9.5,
    height = 4,
    scale = 1,
...
write_csv(
    cume_excess_deaths_hisp %>%
        select(death_cat,
               ...
               upper,
               lower),
    here("output", "figS07_data_educ_and_hispanic.csv")
)
