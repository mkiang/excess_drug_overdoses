## 08_sens_analysis_stratified_age.R ----
##
## This file contains all the code necessary to do a age*race/ethnicity analysis
## of our results stratifying the models by age. Note that due to sparse data
...
## Step 1. Summarize raw data to weekly level stratified by age ----
if (!fs::file_exists(here::here("data", "supp_summarized_weekly_deaths_by_age.RDS"))) {
    ### Data ----
    ...
    flagged_df <- flagged_df %>% 
        dplyr::mutate(
            age_grp = dplyr::case_when(
                dplyr::between(age, 0, 17) ~ "0 to 17",
                dplyr::between(age, 18, 24) ~ "18 to 24",
                dplyr::between(age, 25, 39) ~ "25 to 39",
                dplyr::between(age, 40, 64) ~ "40 to 64",
                dplyr::between(age, 65, Inf) ~ "65 and up",
                TRUE ~ NA_character_
            )
        )
    ...
    weekly_df <- flagged_df %>%
        ...
        dplyr::mutate(date = max(dod),
               date_start = min(dod)) %>%
        dplyr::ungroup() %>%
        dplyr::arrange(race, educ, age_grp, date_start) %>%
        dplyr::select(-id,
               -age,
               -dod,
               -sex,
               ...
               -month, 
               -county_fip) %>%
        dplyr::group_by(race, educ, age_grp, date, date_start, week_from_start)
    ...
    weekly_summarized_df <- weekly_df %>%
        ...
        tidyr::pivot_longer(cols = covid:meth,
                     names_to = "death_type",
                     values_to = "n_deaths") %>%
        dplyr::arrange(death_type, race, educ, age_grp, date)
    
    ## Save ----
    saveRDS(weekly_summarized_df, 
            here::here("data", "supp_summarized_weekly_deaths_by_age.RDS"),
            compress = "xz")
}

## Step 2. Get matching population data by race/ethnicity and sex ----
if (!fs::file_exists(here::here("data", "california_pop_by_race_age.RDS"))) {
    ### Import raw data ----
    ca_pops <- purrr::map_df(.x = 2015:2019,
                      .f = ~ read_acs_microdata_by_age(here::here(
                          "data_raw",
                          "acs_microdata_pulls",
                          "by_age", 
                          sprintf("ca_pop_race_age_group_%i.csv", .x)
                      ))) %>% 
        dplyr::mutate(GEOID = "06",
               st_name = "California") %>%
        dplyr::distinct()
    
    ## Munge to race*age population estimates by year ----
    all_combos <- dplyr::bind_rows(
        ca_pops %>% 
            dplyr::filter(race != "all") %>% 
            dplyr::mutate(race = "hispanic") %>% 
            dplyr::group_by(GEOID, st_name, year, race, educ, age_grp) %>% 
        ...
        dplyr::arrange(GEOID, st_name, educ, race, age_grp, year)
    saveRDS(all_combos, here::here("data", "california_pop_by_race_age.RDS"))
} 
...
if (!all(fs::file_exists(c(here::here("data", "pop_by_race_sex_extrapolated.RDS"),
                       ...
                       here::here("data", "supp_analytic_weekly_df_age.RDS"))))) {
    pop_df <- readRDS(here::here("data", "california_pop_by_race_age.RDS"))
    weekly_df <- readRDS(here::here("data", "supp_summarized_weekly_deaths_by_age.RDS"))
    
    ### Extrapolate out to 2020 ----
    pop_df <- dplyr::bind_rows(
        pop_df,
        pop_df %>% 
            dplyr::group_by(GEOID, st_name, race, educ, age_grp) %>% 
            dplyr::mutate(pop_change = pop[year == 2019] / pop [year == 2018]) %>% 
            dplyr::arrange(st_name, race, educ, age_grp, year) %>% 
    ...
    weekly_df <- weekly_df %>% 
        ...
        dplyr::filter(!is.na(race),
               !is.na(age_grp)) %>% 
        dplyr::arrange(race, educ, age_grp, death_type, date) %>% 
        dplyr::ungroup()
    
    saveRDS(pop_df, here::here("data", "pop_by_race_age_extrapolated.RDS"))
    saveRDS(interpolated_data, here::here("data", "pop_by_race_age_interpolated.RDS"))
    saveRDS(weekly_df, here::here("data", "supp_analytic_weekly_df_age.RDS"))
}
...
### Data ----
analytic_df <- readRDS(here::here("data", "supp_analytic_weekly_df_age.RDS")) %>% 
...
if (!file.exists(here::here("data", "supp_model_forecasting_errors_age.RDS"))) {
    ...
    saveRDS(forecast_errors, here::here("data", "supp_model_forecasting_errors_age.RDS"), compress = "xz")
} else {
    forecast_errors <- readRDS(here::here("data", "supp_model_forecasting_errors_age.RDS"))
}

### Summarize the out of sample errors ----
if (!file.exists( here::here("data", "supp_model_error_summary_age.RDS"))) {
    error_summary <- forecast_errors %>%
        ...
        dplyr::group_by(race, educ, age_grp, death_type, n_harmonic, n_knot) %>%
        ...
        dplyr::group_by(race, educ, age_grp, death_type) %>% 
        dplyr::mutate(model_rank = dplyr::row_number(mse)) %>% 
        dplyr::ungroup() %>% 
        dplyr::filter(n_predictions > 0) %>% 
        dplyr::arrange(race, educ, age_grp, death_type, model_rank) %>% 
        categorize_race() %>% 
        categorize_educ() %>% 
        categorize_death()
    
    saveRDS(error_summary, here::here("data", "supp_model_error_summary_age.RDS"))
} else {
    error_summary <- readRDS(here::here("data", "supp_model_error_summary_age.RDS"))
}
...
if (!fs::file_exists(here::here("data", "supp_expected_deaths_age.RDS"))) {
    expected_deaths <- purrr::map_dfr(
        ...
        categorize_educ() %>% 
        categorize_death()
    saveRDS(expected_deaths, here::here("data", "supp_expected_deaths_age.RDS"))
} else {
    expected_deaths <- readRDS(here::here("data", "supp_expected_deaths_age.RDS"))
}

### Calculate excess mortality over time for each group ----
if (!fs::file_exists(here::here("data", "supp_excess_deaths_age.RDS"))) {
    excess_deaths <- purrr::map_dfr(
        ...
        categorize_educ() %>% 
        categorize_death()
    saveRDS(excess_deaths, here::here("data", "supp_excess_deaths_age.RDS"))
} else {
    excess_deaths <- readRDS(here::here("data", "supp_excess_deaths_age.RDS"))
}

### Calculate cumulative excess mortality for each group ----
if (!fs::file_exists(here::here("data", "supp_cume_excess_deaths_age.RDS"))) {
    cume_excess_deaths <- purrr::map_dfr(
        ...
        categorize_educ() %>% 
        categorize_death()
    saveRDS(cume_excess_deaths, here::here("data", "supp_cume_excess_deaths_age.RDS"))
}
