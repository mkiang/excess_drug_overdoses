## 11_sens_analysis_2019_excess_deaths.R ----
## 
...
## testable. Here, we will use our main excess mortality model on 2019 (i.e.,
## in the absence of COVID-19) to see if we can produce reasonable excess
## mortality deaths.  
...
## Constants ----
FORECAST_START <- as.Date("2019-01-05")
LAST_WEEK <- as.Date("2019-12-28")
FREQUENCY <- 52
...
if (!file.exists(here::here("data", "supp_2019_model_forecasting_errors.RDS"))) {
    ...
    saveRDS(forecast_errors, here::here("data", "supp_2019_model_forecasting_errors.RDS"), compress = "xz")
} else {
    forecast_errors <- readRDS(here::here("data", "supp_2019_model_forecasting_errors.RDS"))
}

### Summarize the out of sample errors ----
if (!file.exists(here::here("data", "supp_2019_model_error_summary.RDS"))) {
    ...
    saveRDS(error_summary, here::here("data", "supp_2019_model_error_summary.RDS"))
} else {
    error_summary <- readRDS(here::here("data", "supp_2019_model_error_summary.RDS"))
}
...
if (!fs::file_exists(here::here("data", "supp_2019_expected_deaths.RDS"))) {
    expected_deaths <- furrr::future_map_dfr(
        ...
        .f = ~ {
            ...
            ## need to calculate var(expected): 
            ##  expected^2 * log_expected_se^2 is the var(expected_t)
            ## Then over time, take the sum of expected and 
            ##  the variance is the sum of var(expected_t)
            ##  then take 1.96 * sqrt(var(expected))
        }
    ) %>% 
        categorize_race() %>% 
        categorize_educ() %>% 
        categorize_death()
    saveRDS(expected_deaths, here::here("data", "supp_2019_expected_deaths.RDS"))
} else {
    expected_deaths <- readRDS(here::here("data", "supp_2019_expected_deaths.RDS"))
}


## Calculate excess mortality over time for each group ----
if (!fs::file_exists(here::here("data", "supp_2019_excess_deaths.RDS"))) {
    excess_deaths <- furrr::future_map_dfr(
        ...
        categorize_educ() %>% 
        categorize_death()
    saveRDS(excess_deaths, here::here("data", "supp_2019_excess_deaths.RDS"))
} else {
    excess_deaths <- readRDS(here::here("data", "supp_2019_excess_deaths.RDS"))
}

## Calculate cumulative excess mortality for each group ----
if (!fs::file_exists(here::here("data", "supp_2019_cume_excess_deaths.RDS"))) {
    cume_excess_deaths <- furrr::future_map_dfr(
        ...
        .f = ~ {
            ...
            educ_x <-
                best_models$educ[.x]
            death_x <-
                best_models$death_type[.x]
            n_harmonics <-
                best_models$n_harmonic[.x]
            n_knots <-
                best_models$n_knot[.x]
    ...
    saveRDS(cume_excess_deaths,
            here::here("data", "supp_2019_cume_excess_deaths.RDS"))
} 
